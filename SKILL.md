...
6. **三步评分**：① 否决过滤 → ② 七维评分（100分制+加减分）→ ③ 八维爆款诊断（找短板，不打分）。详见”选题评分系统”。
7. **最终输出**：默认只输出最终 3-5 个推荐；如需 TOP10，先给表格，并标出”观察/不建议主写”的理由。
...
| ② 七维评分 | 排名选题优先级 | account_profile 七维框架 | 综合评分 X/115 |
...
基于 `account_profile.md` 的公众号专属评分框架：
...
```
综合评分 = 七维总分(0-100) + 加分项(0~+15) - 减分项(0~-18)
理论范围：0 ~ 115
```
...
- 优先选择有实时新闻支撑的选题
- 标题避免过度标题党，保持内容真实性
- 评分使用七维框架（100分制+加减分），不要用已废弃的 120 分制
